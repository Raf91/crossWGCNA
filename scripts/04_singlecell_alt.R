...
write.xlsx(rankprod, file="results/rankprod_stroma_alt.xlsx", rowNames=T)
...
pdf("results/singlecell_scatter_stroma_alt.pdf",5,4)
...
pdf("results/singlecell_boxplot_stroma_alt.pdf",3,5)
...
write.xlsx(rankprod, file="results/rankprod_epi_alt.xlsx", rowNames=T)
rownames(rankprod)[order(rankprod[intersect(rankprod[,1], extracellular),2], decreasing=F)][1:10]
...
df<-data.frame(rankproduct=log2(rankprod[inboth,2]), sc=coef_gsea_sc[inboth])
pdf("results/singlecell_scatter_epi_alt.pdf",5,4)
ggplot(df, aes(x=rankproduct,y=sc))+geom_hex(bins=60)+ylab(label = "Comm. score single cell")+xlab(label = "Rank product")+geom_smooth(method="lm")+theme_classic()
dev.off()
...
my_comparisons <- list( c("Top", "Bottom"))
pdf("results/singlecell_boxplot_epi_alt.pdf",3,5)
ggboxplot(df, x="dir", y="sc")+stat_compare_means(comparisons=my_comparisons,label = "p.signif")+ylab("Comm. score single cell")+xlab("")
dev.off()
...
m_list = m_df %>% split(x = .$gene_symbol, f = .$gs_name)
  ...
  if(nrow(fgseaRes)>0){
    write.xlsx(as.data.frame(fgseaRes[,c(1,2,3,6,7)]), paste("results/fgsea_ratio_stroma_sc_alt.xlsx", sep=""))
  }
  ...
  if(nrow(fgseaRes2)>0){
    write.xlsx(as.data.frame(fgseaRes2[,c(1,2,3,6,7)]), paste("results/fgsea_ratio_epi_sc_alt.xlsx", sep=""))
}
