...
Adjacency<-function(data, comp1="_1",comp2="_2", Adj_type="signed", cortype="spearman", pval="none", thr=0.05, beta=6, sign_list=1, which.sign="none" ){
  ...
  avgpath<-t(t(avgpath*A[Idx])*A[Idx])
...
clusteringWGCNA<-function(A, data, comp1="_1", comp2="_2", TOM=T, ds=1, crossOnly=T){
  ...
  genes1<-gsub(comp1, "", rownames(similarity)[grep(comp1, rownames(similarity))])
  genes2<-gsub(comp2, "", rownames(similarity)[grep(comp2, rownames(similarity))])
  Idx1<-cbind(grep(comp1, rownames(similarity)), grep(comp2, rownames(similarity))[match(genes1, genes2)])
  similarity[Idx1]<-0
  Idx2<-cbind(grep(comp2, rownames(similarity)), grep(comp1, rownames(similarity))[match(genes1, genes2)])
  similarity[Idx2]<-0
...
crossWGCNA<-function(data, Adj_type="signed", cortype="spearman", pval="none", thr=0.05, beta=6, comp1="_1", comp2="_2", doTOM=T, ds=1, crossOnly=T, sign_list=1, which.sign="comp2"){
  comp1<-paste(comp1, "$", sep="")
  comp2<-paste(comp2, "$", sep="")
...
network<-function(data, Adj_type="signed", cortype="spearman", pval="none", thr=0.05, beta=6, comp1="_1", comp2="_2", doTOM=T, ds=1, crossOnly=T, sign_list=1, which.sign="comp2"){
  comp1<-paste(comp1, "$", sep="")
  comp2<-paste(comp2, "$", sep="")
  Adj<-Adjacency(data=data, Adj_type=Adj_type, cortype=cortype,pval=pval, thr=thr, beta=beta, sign_list=sign_list, which.sign=which.sign )
  k<-degrees(A=Adj, comp1=comp1, comp2=comp2)
