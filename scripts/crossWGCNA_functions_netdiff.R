...
Adjacency<-function(data, comp1="_1",comp2="_2", Adj_type="signed", cortype="spearman", pval="none", thr=0.05, beta=6, sign_list=1, compartment_sel="none" , selgenes=NA){
  ...
  #remove average conserved interactions
#assume che i geni siano nello stesso ordine nei due compartimenti
  avgpath<-matrix(ncol=nrow(A)/2, nrow=nrow(A)/2)
  for(x in 1:(nrow(A)/2)){
     avgpath[x,]<-(A[grep(comp1, rownames(A))[x], grep(comp1, rownames(A))]+A[grep(comp2, rownames(A))[x], grep(comp2, rownames(A))])/2
  }
...
A<-A/2
  ...
  if(compartment_sel=="comp2"){
    ...
    names(sign_list)<-paste(names(sign_list), gsub("\\$", "", comp2), sep="")
     ...
     A<-A[c(grep(comp1, rownames(A)), which(rownames(A) %in% selgenes)), c(grep(comp1, rownames(A)), which(rownames(A) %in% selgenes))]
     A[grep(comp2, rownames(A)), grep(comp1, rownames(A))]<-A[grep(comp2, rownames(A)), grep(comp1, rownames(A))]*(sign_list[rownames(A)[grep(comp2, rownames(A))]])
    A[grep(comp1, rownames(A)), grep(comp2, rownames(A))]<-t(A[grep(comp2, rownames(A)), grep(comp1, rownames(A))]*(sign_list[rownames(A)[grep(comp2, rownames(A))]]))
    ...
    A<-A[c(which(rownames(A) %in% selgenes), grep(comp2, rownames(A))), c(which(rownames(A) %in% selgenes), grep(comp2, rownames(A)))]
    A[grep(comp1, rownames(A)), grep(comp2, rownames(A))]<-A[grep(comp1, rownames(A)), grep(comp2, rownames(A))]*(sign_list[rownames(A)[grep(comp1, rownames(A))]])
    A[grep(comp2, rownames(A)), grep(comp1, rownames(A))]<-t(A[grep(comp1, rownames(A)), grep(comp2, rownames(A))]*(sign_list[rownames(A)[grep(comp1, rownames(A))]]))
  }
...
crossWGCNA<-function(data, Adj_type="signed", cortype="spearman", pval="none", thr=0.05, beta=6, comp1="_1", comp2="_2", doTOM=T, ds=1, crossOnly=T, sign_list=1, compartment_sel="none", selgenes=NA){
  comp1<-paste(comp1, "$", sep="")
  comp2<-paste(comp2, "$", sep="")
  Adj<-Adjacency(data=data, Adj_type=Adj_type, cortype=cortype,pval=pval, thr=thr, beta=beta, comp1=comp1, comp2=comp2, sign_list=sign_list, compartment_sel=compartment_sel, selgenes=selgenes )
...
network<-function(data, Adj_type="signed", cortype="spearman", pval="none", thr=0.05, beta=6, comp1="_1", comp2="_2", doTOM=T, ds=1, crossOnly=T, sign_list=1, compartment_sel="none", selgenes=NA){
  comp1<-paste(comp1, "$", sep="")
  comp2<-paste(comp2, "$", sep="")
  Adj<-Adjacency(data=data, Adj_type=Adj_type, cortype=cortype,pval=pval, thr=thr, beta=beta, comp1=comp1, comp2=comp2, sign_list=sign_list, compartment_sel=compartment_sel, selgenes=selgenes )
  k<-degrees(A=Adj, comp1=comp1, comp2=comp2)
