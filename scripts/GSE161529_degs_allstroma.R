...
TNBCstr<-setdiff(colnames(mergedTNBC), TNBCepi)
...
save(averagedEpiTNBC,  file="averagedEpiTNBC_alt.RData")
save(averagedStromaTNBC,  file="averagedStromaTNBC_alt.RData")

load("/home/aurora.savino/crossWGCNA/mergedHER2.RData")

HER2str<-setdiff(colnames(mergedHER2), HER2epi)
...
save(averagedEpiHER2,  file="averagedEpiHER2_alt.RData")
save(averagedStromaHER2,  file="averagedStromaHER2_alt.RData")

load("/home/aurora.savino/crossWGCNA/mergedER.RData")

ERstr<-setdiff(colnames(mergedER), ERepi)
...
save(averagedEpiER,  file="averagedEpiER_alt.RData")
save(averagedStromaER,  file="averagedStromaER_alt.RData")


################################
######Double WGCNA epi stroma tutte le cellule
##############################
load("/home/aurora.savino/crossWGCNA/averagedEpiTNBC_alt.RData")
load("/home/aurora.savino/crossWGCNA/averagedEpiHER2_alt.RData")
load("/home/aurora.savino/crossWGCNA/averagedEpiER_alt.RData")
load("/home/aurora.savino/crossWGCNA/averagedStromaTNBC_alt.RData")
load("/home/aurora.savino/crossWGCNA/averagedStromaHER2_alt.RData")
load("/home/aurora.savino/crossWGCNA/averagedStromaER_alt.RData")
...
save(degsc, file="degsc_allstroma.RData")
...
###filtering data
rownames(averagedEpiER)<-rownames(TNmerge)
rownames(averagedStromaER)<-rownames(TNmerge)

genes<-intersect(c(which(apply(averagedStromaER,1,var)>=quantile(apply(averagedStromaER,1,var),0.5))),
                c(which(apply(averagedEpiER,1,var)>=quantile(apply(averagedEpiER,1,var),0.5))))
stroma<-averagedStromaER[genes,]
epi<-averagedEpiER[genes,]
...
#Functions da A_L.R
degsc<-network(data=data_merged, Adj_type="signed", cortype="pearson", pval="none", thr=0.05, beta=6, comp1="_1", comp2="_2", doTOM=T, ds=4, crossOnly=T)
save(degsc, file="degsc_alt_ER_allstroma.RData")
