...
for(i in 1:length(degs)){
    for(j in 1:length(degs)){
        incommon<-intersect(names(degs[[i]][[1]][[1]])[grep("tis1", names(degs[[i]][[1]][[1]]))], 
        names(degs[[j]][[1]][[1]])[grep("tis1", names(degs[[j]][[1]][[1]]))])
corrs[i,j]<-cor(degs[[i]][[1]][[1]][incommon], degs[[j]][[1]][[1]][incommon])
    }
}
...
for(i in 1:length(degs_old)){
    for(j in 1:length(degs_old)){
        ...
        names(degs_old[[j]][[1]][[1]])[grep("tis1", names(degs[[j]][[1]][[1]]))])
