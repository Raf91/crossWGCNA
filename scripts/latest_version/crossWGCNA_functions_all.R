...
Adjacency<-function(data, method=c("netdiff", "selfloop"), comp1="_1",comp2="_2", Adj_type="signed", cortype="spearman", pval="none", thr=0.05, beta=6,
  ...
  if(method=="selfloop"){
    A<-Adjacency_sl(data=data, comp1=comp1,comp2=comp2, Adj_type=Adj_type, cortype=cortype, pval=pval, thr=thr, beta=beta, sign_list=sign_list, compartment_sel=compartment_sel, selgenes=selgenes )
  } else if(method=="netdiff"){
    A<-Adjacency_nd(data=data, comp1=comp1,comp2=comp2, Adj_type=Adj_type, cortype=cortype, pval=pval, thr=thr, beta=beta, sign_list=sign_list, compartment_sel=compartment_sel, selgenes=selgenes )
  }
  return(A)
}
...
Adjacency_sl <-function(data, comp1="_1",comp2="_2", Adj_type="signed", cortype="spearman", pval="none", thr=0.05, beta=6, sign_list = 1,
  ...
  if (compartment_sel == "comp2") {
    ...
    A[grep(comp1, rownames(A)), grep(comp2, rownames(A))] <-
      t(A[grep(comp2, rownames(A)), grep(comp1, rownames(A))])
    ...
    A[grep(comp2, rownames(A)), grep(comp1, rownames(A))] <-
      t(A[grep(comp1, rownames(A)), grep(comp2, rownames(A))] )
  }
  ####
