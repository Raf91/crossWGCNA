Adjacency <- function(data, method=c("netdiff", "selfloop"),
                    comp1="_1",comp2="_2", Adj_type="signed",
...
Adjacency_sl <- function(data, comp1="_1", comp2="_2", Adj_type="signed",
  ...
  match_comp1 <- grep(comp1, rownames(A))
  match_comp2 <- grep(comp2, rownames(A))
  ...
  if (compartment_sel == "comp2") {
    ...
    A <- A[c(match_comp1, which(rownames(A) %in% selgenes)),
           c(match_comp1, which(rownames(A) %in% selgenes))]

    A[match_comp2, match_comp1] <- A[match_comp2, match_comp1]*(sign_list[rownames(A)[match_comp2]])
    A[match_comp1, match_comp2] <- t(A[match_comp2, match_comp1])
    ...
    A <- A[c(which(rownames(A) %in% selgenes), match_comp2),
           c(which(rownames(A) %in% selgenes), match_comp2)]

    A[match_comp1, match_comp2] <- A[match_comp1, match_comp2]*(sign_list[rownames(A)[match_comp1]])
    A[match_comp2, match_comp1] <- t(A[match_comp1, match_comp2])
  }
...
Adjacency_nd <-
  function(data,
    ...
    match_comp1_orig <- grep(comp1, rownames(A_orig))
    match_comp2_orig <- grep(comp2, rownames(A_orig))
    ...
    for (x in 1:(nrow(A) / 2)) {
      avgpath[x, ] <- (A[match_comp1[x], match_comp1]+A[match_comp2[x], match_comp2])/2
    }

    ### removes average conserved interactions
    A[match_comp1, match_comp2] <- A[match_comp1, match_comp2] - avgpath
    A[match_comp2, match_comp1] <- A[match_comp2, match_comp1] - avgpath

    ### take the lowest absolute value
    diff <- abs(A_orig[match_comp1_orig, match_comp2_orig])-abs(A[match_comp1, match_comp2])
    A[match_comp1, match_comp2][diff<0] <- A_orig[match_comp1_orig, match_comp2_orig][diff<0]

    diff <- abs(A_orig[match_comp2_orig, match_comp1_orig])-abs(A[match_comp2, match_comp1])
    A[match_comp2, match_comp1][diff<0] <- A_orig[match_comp2_orig, match_comp1_orig][diff<0]
...
clusteringWGCNA <- function(A, data, comp1="_1", comp2="_2",
                            TOM=T, ds=1, crossOnly=T)
  ...
  if (crossOnly == T) {
    A[match_comp1, match_comp1] <- 0
    A[match_comp2, match_comp2] <- 0
  }

  #remove self loops
  genes1 <- gsub(comp1, "", rownames(A)[match_comp1])
  genes2 <- gsub(comp2, "", rownames(A)[match_comp2])

  Idx1 <- cbind(match_comp1, match_comp2[match(genes1, genes2)])
  A[Idx1] <- 0

  Idx2 <- cbind(match_comp2, match_comp1[match(genes2, genes1)])
  A[Idx2] <- 0
...
degrees <- function(A, comp1 = "_1", comp2 = "_2") {
  ...
  kTot <- rowSums(A)
  kInt_1 <- rowSums(A[match_comp1, match_comp1])
  kInt_2 <- rowSums(A[match_comp2, match_comp2])
  kExt_1 <- rowSums(A[match_comp1, match_comp2])
  kExt_2 <- rowSums(A[match_comp2, match_comp1])

  k <- list(kInt1=kInt_1,
            kInt2=kInt_2,
            kExt1=kExt_1,
            kExt2=kExt_2,
            kTot1=kTot[match_comp1],
            kTot2=kTot[match_comp2])
...
crossWGCNA <- function(data, method="netdiff", Adj_type = "signed",
                       cortype = "spearman", pval = "none", thr = 0.05,
                       beta = 6, comp1 = "_1",comp2 = "_2",doTOM = T,
                       ds = 1, crossOnly = T, sign_list = 1, compartment_sel = "none",
                       selgenes = NA) {
  ...
  clusters <- clusteringWGCNA(A=Adj,
                              data=data,
                              comp1=comp1,
                              comp2=comp2,
                              TOM=doTOM,
...
network <- function(data, method="netdiff", Adj_type = "signed", cortype = "spearman",
                    pval = "none", thr = 0.05, beta = 6, comp1 = "_1", comp2 = "_2",
                    sign_list = 1, compartment_sel = "none", selgenes = NA) {
...
degrees_mod <- function(data, method = "netdiff", modules, Adj_type = "signed",
                        cortype = "spearman", pval = "none", thr = 0.05, beta = 6,
                        comp1 = "_1", comp2 = "_2") {
  ...
  for(i in 1:length(unique(modules))){
    ...
    Adj <- Adjacency(data=data[c(genes_comp1, genes_comp2),],
                     method=method,
                     comp1=comp1,
                     comp2=comp2,
                     Adj_type=Adj_type,
...
cytoscape_net <- function(adjacency, dataset=data,
                          gene, comp1, comp2, num, corr="spearman"){
  ...
  df <- data.frame(Source=c(rep(gene, length(inter_edges)),
                 ...
                 Weight=c(inter_edges,intra_1,intra_2),
...
ST_spots_coords <- function(data, br=1000){
  ...
  #transform y coordinates so that they represent the height of an equilateral triangle
  y_bin <- (y_bin*sqrt(3))
...
#spots_dist
#
ST_expr_smooth <- function(expr_data, coords, max_dist=5,
                           spots_class, sel_class=c("Epi", "Stroma")){
...
ST_merged_dataset <- function(sel_spots, coords, averaged_expr_all,
                              var_thr=0.75, comp1="_tis1", comp2="_tis2"){
...
ST_plot_expr <- function(gene, averaged_expr_all, coords,
                         included_spots, tis1_spots, tis2_spots, midpoints){

  midpoints_x <- midpoints[[1]]
  midpoints_y <- midpoints[[2]]

  df <- data.frame(x_coord= c(x_bin[tis1_spots],x_bin[tis2_spots], x_bin[included_spots]),
                   y_coord= c(-(y_bin[tis1_spots]), -(y_bin[tis2_spots]), -y_bin[included_spots]),
  ...
  p <- ggplot(data=df, aes(x=x_coord, y=y_coord, colour=gene))+
    geom_point()+
...
ST_plot_comm <- function(gene1, gene2, averaged_expr_all,
                         coords, included_spots, sel_spots,
  ...
  df_midpoint <- data.frame(x_coord= midpoints_x,
                            y_coord= midpoints_y,
                            comm_score=averaged_expr_all[gene1,included_spots_tis1]*
                              averaged_expr_all[gene2,included_spots_tis2])

  p <- ggplot(data=df, aes(x=x_coord, y=y_coord)) +
...
ST_weighted_mod <- function(modules, kw, mod_sel, averaged_expr_all,
